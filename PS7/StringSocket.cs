...
namespace CustomNetworking
{
    ...
    public class StringSocket
    {
        ...
        //receiveQueue keeps the call back and payload of receive request in the order they were receieved. 
        ...
        private void sendCallback(IAsyncResult ar)
        {
            ...
            if (!m_isConnected)
            {
                sendData.m_callback(new ObjectDisposedException("Remote client disconnected"), sendData.m_payload);
                return;
            }
        ...
        {
            if (m_isClosed || !m_isConnected)
                return; // Check if we are closed. If so, stop the receive process.
        ...
        private void ReceiveCompleteString(string str)
        {
            ...
            lock (m_receiveQueue)
            {
                if (m_receiveQueue.Count > 0)
                {
                    ...
                    ReceiveData receiveData = m_receiveQueue.Dequeue();
                    Task callbackTask = Task.Factory.StartNew(
                        () => { receiveData.m_callback(str, null, receiveData.m_payload); });
                }
                else
                {
                    //if there aren't any pending receives, store the extra strings we received locally
                    m_unrequestedStringQueue.Enqueue(str);
                }
            }
        }
        ...
        public void Close()
        {
            ...
            lock(m_sendQueue)
            {
                if(m_sendQueue.Count == 0 && m_isConnected)
                {
